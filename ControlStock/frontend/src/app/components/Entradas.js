...
import { ArrowDownToLine, Calendar, Package, User, MapPin, Plus, Trash2, Save, ShoppingCart, X } from 'lucide-react';
          ...
          {entradas?.length === 0 ? (
            ...
            <div className="space-y-4">
              {entradas?.slice(0, 10).map((entrada) => (
                <div key={entrada.id} className="border border-gray-100 rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    {entrada.observaciones && (
                      <p className="text-xs text-gray-400 mt-2 italic">"{entrada.observaciones}"</p>
                    )}
                  </div>
                </div>
              ))}
